...
# Works if members didn't choose one of the language button
    ...
    def random_profile_sender(chat_id):
        ...
        dislike_emoji = data.get_bot_messages('dislike', lang=lang)
        zzz_emoji = data.get_bot_messages('zzz', lang=lang)
    ...
    def press_four_buttons(message, lang, member, main_menu_buttons):
        ''' Вызывается когда пользователь нажимает на одну из 4 кнопок(like_emoji, send_message_emoji, dislike, zzz) главного меню '''
